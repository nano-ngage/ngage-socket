...
ssh $DEPLOY_USER@$DEPLOY_HOST 'docker run -d --restart=always --link ngage-db:ngagedb -e DBIP="ngagedb" --name ngage-socket -p 5500:5500 nanongage/ngage-socket:latest'
