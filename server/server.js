...
nsp.on('connection', function (socket) {
  ...
  socket.on('start', function (data) {
    ...
    fetch(url + 'qByS/' + data.room).then(res => res.json()).then(questions => {
      ...
      socket.emit('questions', 'Server is unavailable');
    })
  });
  socket.on('askQ', function (data) {
    fetch(url + 'aByQ/' + data.question.questionID).then(handleErrors).then(res => res.json()).then(answers => {
      nsp.in(data.room).emit('answers', {answers: answers, question: data.question});
    }).catch(e => {
      socket.emit('answers', 'Server is unavailable');
    })

  });
  socket.on('submitResponse', data => {
    nsp.in(data.room).emit('resp', data);
    fetch(url + 'r', { method: 'POST', headers: {"Content-Type": "application/json"}, mode: 'cors',body: JSON.stringify(data) }).catch(e => {
      socket.emit('responseSubmit', 'Server is unavailable');
    });
  });
});
